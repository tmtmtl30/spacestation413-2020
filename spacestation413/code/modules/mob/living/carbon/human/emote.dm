...
/datum/emote/living/carbon/fart/run_emote(mob/living/carbon/user, params)
	...
	if(!message)
		message = pick(
			...
			"unleashes [user.p_their()] unholy rectal vapor!",
			"assblasts gently.",
			...
			"farts so loud it startles [user.p_them()]!",
	...
	if(istype(Y))
		...
		if(lose_butt)
			...
			user.visible_message("<span class='warning'><b>[user]</b> blows [user.p_their()] ass off!</span>", "<span class='warning'>Holy shit, your butt flies off in an arc!</span>")
		else
...
/datum/emote/living/carbon/human/superfart/run_emote(mob/living/carbon/human/user, params)
	...
	if(istype(Y))
		...
		switch(fart_type)
			...
			if(2)
				user.visible_message("<span class='warning'><b>[user]</b> rips [user.p_their()] ass apart in a massive explosion!</span>", "<span class='warning'>Holy shit, your butt goes supernova!</span>")
			...
			if(3)
				...
				//ASS BLAST USA
				user.visible_message("<span class='warning'><b>[user]</b> blows [user.p_their()] ass off with such force, they explode!</span>", "<span class='warning'>Holy shit, your butt flies off into the galaxy!</span>")
				playsound(user, 'spacestation413/sound/effects/superfart.ogg', 75, extrarange = 255, pressure_affected = FALSE)
