...
/mob/living/simple_animal/hostile/floor_cluwne
	...
	del_on_death = TRUE
	pass_flags = ALL //it's practically a ghost when unmanifested (under the floor)
...
/mob/living/simple_animal/hostile/floor_cluwne/Initialize()
	...
	if(!current_victim)
		Acquire_Victim()
	poi = new(src)
	setMovetype(FLYING) // movement jank part 0: the prequal. this is to stop the cluwne from breaking GLASS TABLES AS IT WALKS OVER THEM ARE YOU FUCKING KIDDING ME
...
// movement jank part 1. ensures that anything can pass us (unless it uses CanPassThrough, but i don't THINK anything really does)
/mob/living/simple_animal/hostile/floor_cluwne/CanAllowThrough(atom/movable/mover, turf/target)
	..() // the fact that we need to call the parent proc here is fucking stupid.
	// the parent proc returns !density. why do i need to call it if it does fucking NOTHING
	return TRUE

// movement jank part 2. you're not supposed to override this proc, but i'm going to, to stop the cluwne from OPENING DOORS
/mob/living/simple_animal/hostile/floor_cluwne/Bump(atom/A)
	return
...
/mob/living/simple_animal/hostile/floor_cluwne/Life()
	...
	if(get_dist(src, current_victim) > 9 && !manifested &&  !is_type_in_typecache(A, invalid_area_typecache))//if cluwne gets stuck he just teleports
		do_teleport(src, T, no_effects = TRUE)
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Acquire_Victim(var/mob/living/carbon/human/specific)
	if(specific && specific.stat != DEAD && specific.has_dna() && !specific.dna.check_mutation(CLUWNEMUT))
		...
		var/list/shuffled_players = shuffle(GLOB.player_list) // copies it, shuffles it, so the check is fair
		for(var/mob/living/carbon/human/H in shuffled_players)// automatically does istype() check on the list, typecasts as well
			var/area/A = get_area(H.loc)
			if(H && H.stat != DEAD && H != current_victim && H.has_dna() && !H.dna.check_mutation(CLUWNEMUT) && !is_type_in_typecache(A, invalid_area_typecache) && is_station_level(H.z))
				current_victim = H
				interest = 0
				stage = STAGE_HAUNT
				return target = current_victim
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Manifest()//handles disappearing and appearance anim
	if(manifested)
		ADD_TRAIT(src, TRAIT_IMMOBILIZED, CLUWNE_MANIFESTED)
		...
		density = FALSE
		REMOVE_TRAIT(src, TRAIT_IMMOBILIZED, CLUWNE_MANIFESTED)
		if(cluwnehole)
			qdel(cluwnehole)
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/On_Stage()
	...
	switch(stage)
		...
		if(STAGE_HAUNT)
			...
			if(prob(5))
				H.playsound_local(src,'spacestation413/sound/misc/bikehorn_creepy.ogg', 4)
		...
		if(STAGE_SPOOK)
			...
			if(prob(2))
				...
				if(current_victim.hud_used)//yay skewium
					...
					var/intensity = 8
					var/matrix/skew = matrix(1, rand(-intensity,intensity), 0, rand(-intensity,intensity), 1, 0) // not sure if this does what i want it to
					var/matrix/newmatrix = skew // why?
